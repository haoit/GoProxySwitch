	...
	try:
		...
		for proxy in part:
			...
			try:
				...
				pass
		out_file = open("listproxy.txt","a")      # e li scrive nel file, aperto come a, per non sovrascrivere proxy gia presenti all'interno
		...
		for tag in soup.find_all(word2, word): # bs4, dopo aver processato la source, trova la parte riguardante le proxylist
			...
			for line in result :
				...
				if ip: # se ha trovato gli ip prosegue
					for x in ip:
						out_file = open("listproxy.txt","a") # scrittura singolo ip nella proxy.txt
						while True:
							out_file.write(x+"\n") # scrive ip uno per uno nel file proxy.txt
							out_file.close()
...
def proxylist(): # funzione per la creazione della proxylist
	...
	proxies = open("listproxy.txt").readlines() # la lista txt presenta doppioni, quindi:
	proxiesp = []
	for i in proxies:
		if i not in proxiesp: # se il proxy non è già presente in proxiesp
			proxiesp.append(i) # li aggiunge in proxiesp
	filepr = open("listproxy.txt", "w") # prima cancella contenuto del file
	filepr.close()
	filepr = open("listproxy.txt", "a") # dopo lo apre in modalità a per non sovrascrivere i proxy
	for i in proxiesp:
		filepr.write(i)             # e scrive
	print("Current IPs in proxylist: %s" % (len(open("listproxy.txt").readlines()))) # per vedere quante lines (e quindi quanti proxy) ci sono nel file
	print ("\nProxylist Updated!\n")

def proxycheckerinit():
	global out_file
	candidate_proxies = open("listproxy.txt").readlines() # vede gli attuali proxy "candidati" lol
	filedl = open("listproxy.txt", "w") # prima cancella contenuto
	filedl.close()
	out_file = open("listproxy.txt", "a") # e poi lo apre non in riscrivibile
	...
	print("\n\nCurrent IPs in proxylist: %s\n" % (len(open("listproxy.txt").readlines()))) # quando finisce tutto printa la quantità di proxy FINALE
...
def main(): # funzione effettiva del programma.
	try:
		out_file = open("listproxy.txt","w") # prima di tutto cancella il contenuto di proxy.txt
		out_file.close()
		...
		proxyget2(url)
		print("Current IPs in proxylist: %s" % (len(open("listproxy.txt").readlines()))) # printa la lunghezza attuale del file, che sarebbe il numero di proxy
		...
		print("Current IPs in proxylist: %s" % (len(open("listproxy.txt").readlines())))

		print ("\nDownloading from various mirrors in progress...")
		for position, url in enumerate(nurls):
			proxyget(url)
			print("Completed downloads: (%s/%s)\nCurrent IPs in proxylist: %s" % (position+1, len(nurls), len(open("listproxy.txt").readlines())))
